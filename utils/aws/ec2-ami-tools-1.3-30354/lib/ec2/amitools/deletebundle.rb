...
class BundleDeleter < AMITool
  ...
  def delete(bucket, file, retry_delete)
    basename = File::basename(file)
    retry_s3(retry_delete) do
      begin
        response = @s3_conn.delete(bucket, basename)
        return if response.success?
        raise "HTTP DELETE returned #{response.code}"
      rescue => e
        raise TryFailed.new("Failed to delete \"#{file}\": #{e.message}")
      end
    end
  end
  ...
  def get_file_list_from_s3(bucket, prefix)
    files_to_delete = []
    response = @s3_conn.list_bucket(bucket, prefix)
    ...
    REXML::XPath.each(REXML::Document.new(response.body), "//Key/text()") do |entry|
      entry = entry.to_s
      files_to_delete << entry if entry =~ /^#{prefix}\.part\./
      files_to_delete << entry if entry =~ /^#{prefix}\.manifest$/
      files_to_delete << entry if entry =~ /^#{prefix}\.manifest\.xml$/
    end
    files_to_delete
  end
  ...
  def delete_bundle(url, bucket, user, pass, manifest, prefix, yes, clear, retry_stuff)
    begin
      # Get the S3 URL.
      ...
      if manifest
        ...
        files_to_delete = get_file_list_from_s3(bucket, prefix)
      end
      ...
      if files_to_delete.empty?
        ...
        if continue
          files_to_delete.each do |file|
            delete(bucket, file, retry_delete)
