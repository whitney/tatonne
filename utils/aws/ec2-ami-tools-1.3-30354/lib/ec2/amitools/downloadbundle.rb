...
class BundleDownloader < AMITool
  ...
  def download_bundle(url,
    ...
    begin
      ...
      manifest_xml = download_manifest(s3_conn, bucket, manifest, manifest_path, privatekey, retry_stuff)
      
      # Download AMI parts.
      get_part_filenames(manifest_xml).each do |filename|
        download_part(s3_conn, bucket, filename, File::join(directory, filename), retry_stuff)
